...
const Onboarding21Screen: React.FC<Props> = ({ navigation }) => {
  const dispatch = useAppDispatch();
  useEffect(() => { dispatch(setCurrentStep(21)); }, [dispatch]);
  // Stars animation config
  const stars = useMemo(
    () => [
      { top: '22%', left: '14%', size: 3, delay: 0 },
      { top: '34%', left: '76%', size: 2, delay: 300 },
      { top: '62%', left: '22%', size: 3, delay: 600 },
      { top: '72%', left: '66%', size: 2, delay: 900 },
      { top: '44%', left: '86%', size: 3, delay: 1200 },
    ],
    []
  );
  ...
  <View style={styles.progress}><ProgressIndicator currentStep={21} totalSteps={30} variant="bars" showStepText={false} /></View>
        <View style={{ width: 40 }} />
      </View>
  ...
  {/* Icon divider */}
  <Icon name="walk" size={36} color="#ffffff" style={styles.iconDivider} />

        <Text style={styles.title}>Rewire your brain</Text>
        <Text style={styles.subtitle}>
          <Text style={styles.emphasis}>Science‑backed practices</Text> help <Text style={styles.emphasis}>rebuild dopamine receptors</Text>
          {' '}and form new, holy habits. With God's help, resilience grows and
          {' '}<Text style={styles.emphasis}>relapse loses its grip</Text>.
        </Text>
  <MiniSectionDots total={9} active={2} />
  <OnboardingButton title="Continue" onPress={() => navigation.navigate('Onboarding22')} variant="primary" style={{ marginTop: 12 }} />
      </View>
    </SafeAreaView>
  );
};
...
export default Onboarding21Screen;
