...
const Onboarding24Screen: React.FC<Props> = ({ navigation }) => {
  const dispatch = useAppDispatch();
  useEffect(() => { dispatch(setCurrentStep(24)); }, [dispatch]);
  const stars = useMemo(
    () => [
  ...
  <View style={styles.progress}><ProgressIndicator currentStep={24} totalSteps={30} variant="bars" showStepText={false} /></View>
        <View style={{ width: 40 }} />
      </View>
      <View style={styles.content}>
        ...
        {/* Icon divider */}
        <Icon name="trophy" size={36} color="#ffffff" style={styles.iconDivider} />

        <Text style={styles.title}>Conquer yourself</Text>
        <Text style={styles.subtitle}>
          <Text style={styles.emphasis}>Know yourself</Text> to overcome. Understand your
          {' '}<Text style={styles.emphasis}>strengths and weaknesses</Text>, earn medals, and track
          {' '}your <Text style={styles.emphasis}>progress</Text> — all by grace, not striving alone.
        </Text>
  <MiniSectionDots total={9} active={5} />
  <OnboardingButton title="Continue" onPress={() => navigation.navigate('Onboarding25')} variant="primary" style={{ marginTop: 12 }} />
      </View>
    </SafeAreaView>
  );
};
...
export default Onboarding24Screen;
