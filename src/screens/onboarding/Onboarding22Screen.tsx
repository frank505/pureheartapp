...
const Onboarding22Screen: React.FC<Props> = ({ navigation }) => {
  const dispatch = useAppDispatch();
  useEffect(() => { dispatch(setCurrentStep(22)); }, [dispatch]);
  // Stars animation config
  const stars = useMemo(
    () => [
      { top: '20%', left: '16%', size: 3, delay: 0 },
      { top: '36%', left: '82%', size: 2, delay: 300 },
      { top: '60%', left: '26%', size: 3, delay: 600 },
      { top: '76%', left: '72%', size: 2, delay: 900 },
      { top: '46%', left: '90%', size: 3, delay: 1200 },
    ],
    []
  );
  ...
  <View style={styles.progress}><ProgressIndicator currentStep={22} totalSteps={30} variant="bars" showStepText={false} /></View>
        <View style={{ width: 40 }} />
      </View>
  ...
  <Icon name="sunny-outline" size={36} color="#fb923c" style={styles.iconDivider} />
        ...
        <Text style={styles.subtitle}>
          Quitting pornography can be hard. <Text style={styles.emphasis}>Daily check‑ins</Text> keep you anchored,
