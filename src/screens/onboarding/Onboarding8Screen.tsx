...
interface Onboarding8ScreenProps { navigation: any }

const Onboarding8Screen: React.FC<Onboarding8ScreenProps> = ({ navigation }) => {
  const dispatch = useAppDispatch();
  const existing = (useAppSelector((s) => s.onboarding.assessmentData?.questions) ?? []) as AssessmentQuestion[];
  const [value, setValue] = useState<string>('');

  useEffect(() => { dispatch(setCurrentStep(8)); }, [dispatch]);
  ...
  const saveAndNext = () => {
    if (!value) return navigation.navigate('Onboarding9');
    const others = existing.filter((q: any) => q?.id !== 'stress_trigger');
    const newQuestion: AssessmentQuestion = {
      id: 'stress_trigger',
      question: 'Do you turn to pornography when stressed?',
      currentAnswer: value,
      icon: '⚡',
      type: 'options',
    };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.progressWrapper}>
          <ProgressIndicator currentStep={8} totalSteps={29} variant="bars" showStepText={false} />
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.subtitle}>Do you turn to pornography when stressed?</Text>
