...
interface Onboarding7ScreenProps { navigation: any }

const Onboarding7Screen: React.FC<Onboarding7ScreenProps> = ({ navigation }) => {
  const dispatch = useAppDispatch();
  const existing = (useAppSelector((s) => s.onboarding.assessmentData?.questions) ?? []) as AssessmentQuestion[];
  const [value, setValue] = useState<string>('');

  useEffect(() => { dispatch(setCurrentStep(7)); }, [dispatch]);
  ...
  const saveAndNext = () => {
    if (!value) return navigation.navigate('Onboarding8');
    const others = existing.filter((q: any) => q?.id !== 'coping_usage');
    const newQuestion: AssessmentQuestion = {
      id: 'coping_usage',
      question: 'Do you use pornography to cope with emotional discomfort?',
      currentAnswer: value,
      icon: '🧠',
      type: 'options',
    };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <LinearGradient colors={[Colors.background.primary, Colors.background.secondary, '#f59e0b']} style={StyleSheet.absoluteFill} />
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.progressWrapper}>
          <ProgressIndicator currentStep={7} totalSteps={30} variant="bars" showStepText={false} />
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.subtitle}>Do you use pornography to cope with emotional discomfort?</Text>
