...
import { getVictoryById, addCommentToVictory, getVictoryComments } from '../store/slices/victorySlice';
...
const VictoryDetailScreen = ({ route, navigation }: any) => {
  ...
  const handleAddComment = () => {
    if (commentBody.trim()) {
      dispatch(addCommentToVictory({ id: victoryId, comment: { body: commentBody } }));
      setCommentBody('');
    }
  };
          ...
          <Surface style={styles.card} elevation={2}>
            <View style={styles.rowBetween}>
              <View style={styles.col}>
                <Text style={styles.title}>{selectedVictory.title}</Text>
                {!!selectedVictory.body && (
                  <Text style={styles.body}>{selectedVictory.body}</Text>
                )}
                <View style={styles.metaRow}>
                  <Chip compact style={styles.metaChip}>{selectedVictory.visibility}</Chip>
                  <Text style={styles.metaText}>{new Date(selectedVictory.createdAt).toLocaleString()}</Text>
                </View>
              </View>
            </View>
          </Surface>
...
export default VictoryDetailScreen;
