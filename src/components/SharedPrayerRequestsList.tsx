...
import { getSharedPrayerRequests } from '../store/slices/prayerRequestSlice';
import { RootState, AppDispatch } from '../store';
import { PrayerRequest } from '../services/prayerRequestService';

const SharedPrayerRequestsList = ({ navigation }: any) => {
  const dispatch = useDispatch<AppDispatch>();
  const { sharedPrayerRequests, loading, error } = useSelector((state: RootState) => state.prayerRequests);

  const load = (page = 1) => {
    dispatch(getSharedPrayerRequests({ page, limit: 10 }));
  };
  ...
  const onEndReached = () => {
    if (!loading && sharedPrayerRequests?.page < sharedPrayerRequests?.totalPages) {
      load(sharedPrayerRequests.page + 1);
    }
  };
  ...
  return (
    <View style={styles.container}>
      {loading && sharedPrayerRequests?.items.length === 0 && (
      ...
      <FlatList
        data={sharedPrayerRequests?.items || []}
        ...
        ListEmptyComponent={!loading ? (
          <View style={{ alignItems: 'center', padding: 16 }}>
            <Text style={{ color: Colors.text.secondary }}>No prayer requests have been shared with you yet.</Text>
          </View>
        ) : null}
      />
    </View>
...
export default SharedPrayerRequestsList;
