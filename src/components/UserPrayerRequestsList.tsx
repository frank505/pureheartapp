...
import { getPrayerRequestsByUserId } from '../store/slices/prayerRequestSlice';
import { RootState, AppDispatch } from '../store';
import { PrayerRequest } from '../services/prayerRequestService';

const UserPrayerRequestsList = ({ navigation }: any) => {
  const dispatch = useDispatch<AppDispatch>();
  const { prayerRequests, loading, error } = useSelector((state: RootState) => state.prayerRequests);
  const { currentUser } = useSelector((state: RootState) => state.user);

  const load = (page = 1) => {
    if (currentUser) {
      dispatch(getPrayerRequestsByUserId({ userId: Number(currentUser.id), page, limit: 10 }));
    }
  };
  ...
  const onEndReached = () => {
    if (!loading && prayerRequests.page < prayerRequests.totalPages) {
      load(prayerRequests.page + 1);
    }
  };
  ...
  return (
    <View style={styles.container}>
      {loading && prayerRequests.items.length === 0 && (
      ...
      <FlatList
        ...
        ListEmptyComponent={!loading ? (
          <View style={{ alignItems: 'center', padding: 16 }}>
            <Text style={{ color: Colors.text.secondary }}>You haven't created any prayer requests yet.</Text>
          </View>
        ) : null}
      />
    </View>
...
export default UserPrayerRequestsList;
